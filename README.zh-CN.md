...
TanVite 是一个面向生产环境的 React 19 模板仓库，定位为可复用的高标准前端工程基线，适合严肃的产品研发场景。它将 Vite、TypeScript、OpenSpec、面向 Codex 与 Claude Code 的共享 `.agents/skills` 技能目录、TanStack Router、TanStack Query、OpenAPI 工具链、Tailwind CSS、自动化测试、代码质量约束以及 GitHub Pages 展示能力整合为一套打磨完整、适合新项目直接起步的现代前端最佳实践组合，并把 spec-driven 工作流和 AI 辅助研发能力一起带入仓库基线。
...
- 直接使用面向 Codex 与 Claude Code 的共享技能目录处理设计、代码审查、测试、Git 工作流和浏览器自动化
...
- 已集成共享 `.agents/skills` 技能目录，同时支持 Codex 与 Claude Code，覆盖设计、评审、测试、提交流程和浏览器自动化
...
| 包管理器 | pnpm |
| 规格工作流 | OpenSpec |
| AI 协作层 | `.agents/skills`、Codex、Claude Code、OPSX commands |
| 路由 | TanStack Router |
| 数据获取 | TanStack Query |
...
当前工作区内置了一套共享技能目录，位于 `.agents/skills/`，并按同一套能力同时服务 Codex 和 Claude Code。
...
- 把共享 `.agents/skills` 目录纳入版本管理，让 Codex 和 Claude Code 始终基于同一套技能协作
